import type {
  ...
  SLASAuthType,
  TokenResponse,
} from '@headless-commerce/types/SlasAuth';
import { ocapiShopAPIBaseUrl } from '@headless-commerce/utils/configuration/ocapi';
...
export const slasSessionBridgeFromDwsid = async ({
  ...
  if (response.status >= 400) {
    throw new Error(`${NETWORK_FAIL_ERROR} - slasSessionBridgeFromDwsid - status:${response.status}`);
  }

  const responseJson = await response.json();

  if (responseJson.status_code && parseInt(responseJson.status_code, 10) >= 400) {
    throw new Error(`${NETWORK_FAIL_ERROR} - slasSessionBridgeFromDwsid - status_code:${responseJson.status_code}`);
  }
...
export const getScapiToken = async ({
  ...
  if (response.status >= 400) {
    throw new Error(`${NETWORK_FAIL_ERROR} - getScapiToken - status:${response.status}`);
  }
...
export const getOcapiCustomerFromDwsid = async ({
  ...
  if (response.status >= 400) {
    throw new Error(`${NETWORK_FAIL_ERROR} - getOcapiCustomerFromDwsid - status:${response.status}`);
  }

  const responseJson = await response.json();

  if (responseJson.status_code && parseInt(responseJson.status_code, 10) >= 400) {
    throw new Error(`${NETWORK_FAIL_ERROR} - getOcapiCustomerFromDwsid - status_code:${responseJson.status_code}`);
  }
...
export const ocapiSessionBridgeFromToken = async ({
  ...
  if (!response.ok || response.status >= 400) {
    throw new Error(`${NETWORK_FAIL_ERROR} - ocapiSessionBridgeFromToken - status:${response.status}`);
  }
...
export const slasRefreshToken = async ({
  ...
  if (response.status >= 400) {
    throw new Error(`${NETWORK_FAIL_ERROR} - slasRefreshToken - status:${response.status}`);
  }
