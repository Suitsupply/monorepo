...
export type GetCartByCustomerQueryVariables = Types.Exact<{
  customerId: Types.Scalars['ID'];
  siteInfo: Types.SiteInfo;
}>;


export type GetCartByCustomerQuery = { getCartByCustomer: Types.Maybe<(
    Pick<Types.Cart, 'currency'>
    & { lineItems: Types.Maybe<Array<Types.Maybe<Pick<Types.CustomMadeLineItem, 'productId' | 'masterProductId' | 'price' | 'priceAfterDiscount' | 'quantity' | 'size'> | Pick<Types.ProductLineItem, 'productId' | 'masterProductId' | 'price' | 'priceAfterDiscount' | 'quantity' | 'size'>>>>, giftCertificateItems: Types.Maybe<Array<Types.Maybe<Pick<Types.GiftCertificateItem, 'price' | 'type'>>>> }
  )> };
